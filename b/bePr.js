...
async function loadInvoiceData(billId, modalElement) {
 try {
  ...
  let whatsAppLnkTxt = mob_client?.replace(".", "") + "&text=" + "Hello, \n*" + name_client?.trim() + "*\n" + "Thank you for choosing us.\n";
  whatsAppLnkTxt += "Here are details of your voucher no." + curr_bill_info.g + "\n";
...
if (clientConfig.show_all_receipts == 1 && sCurrCashInfo && sCurrCashInfo.length > 0) {
  ...
  let formattedArray;
  if (discountAmt != 0) {
    formattedArray = formatNumbers([t_itms_otal.toFixed(2), discountAmt.toFixed(2), advanceAmt.toFixed(2), dueAmt]);
    whatsAppLnkTxt += `\`${formattedArray[0]}\` Total\n`;
    whatsAppLnkTxt += `\`${formattedArray[1]}\` Discount\n`;
    whatsAppLnkTxt += `\`${formattedArray[2]}\` Received\n`;
  } else {
    formattedArray = formatNumbers([t_itms_otal.toFixed(2), advanceAmt.toFixed(2), dueAmt]);
    whatsAppLnkTxt += `\`${formattedArray[0]}\` Total\n`;
    whatsAppLnkTxt += `\`${formattedArray[1]}\` Received\n`;
  }
}
  
  // Add balance and footer
  whatsAppLnkTxt += `\`${dueAmt}\` Balance\n*${ensureUTF(clientConfig.printCompNm)}*`;
     ...
     <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
      ...
      <tbody>
       <tr>
        ...
        <td style="text-align: center; padding: 4px; border: 1px solid #000;">${eye_selected_msrmnt.ra || ''}</td>
        <td style="text-align: center; padding: 4px; border: 1px solid #000;">${eye_selected_msrmnt.rb || ''}</td>
       ...
       <tr>
        ...
        <td style="text-align: center; padding: 4px; border: 1px solid #000;">${eye_selected_msrmnt.re || ''}</td>
        <td style="text-align: center; padding: 4px; border: 1px solid #000;">${eye_selected_msrmnt.rf || ''}</td>
      ...
      <tfoot>
       <tr>
        ...
        <td colspan="4" style="padding: 4px; border: 1px solid #000;">${eye_selected_msrmnt.ri || ''}</td>
       </tr>
       <tr>
    ...
    <div class="left-eye-table" style="border: 1px solid #000;">
     <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
      ...
      <tbody>
       <tr>
        <td style="padding: 4px; border: 1px solid #000; text-align: center;">DV</td>
        <td style="text-align: center; padding: 4px; border: 1px solid #000;">${eye_selected_msrmnt.la || ''}</td>
        <td style="text-align: center; padding: 4px; border: 1px solid #000;">${eye_selected_msrmnt.lb || ''}</td>
       ...
       <tr>
        <td style="padding: 4px; border: 1px solid #000; text-align: center;">NV</td>
        <td style="text-align: center; padding: 4px; border: 1px solid #000;">${eye_selected_msrmnt.le || ''}</td>
        <td style="text-align: center; padding: 4px; border: 1px solid #000;">${eye_selected_msrmnt.lf || ''}</td>
      ...
      <tfoot>
       <tr>
        <td style="padding: 4px; border: 1px solid #000; text-align: center;">ADD</td>
        <td colspan="4" style="padding: 4px; border: 1px solid #000;">${eye_selected_msrmnt.li || ''}</td>
       </tr>
       <tr>
        <td style="padding: 4px; border: 1px solid #000; text-align: center;">PD</td>
...
function applyPaperSettings(modalElement) {
 ...
 if (previewMain && paperBackground && printContentArea) {
  ...
  printContentArea.style.width = contentDisplayWidth + 'px';
  printContentArea.style.height = contentDisplayHeight + 'px';
...
function formatNumbers(numbers) {
 ...
 return numbers.map(num => {
  ...
  return '_'.repeat(Math.max(0, spacesNeeded)) + strNum;
 });
}

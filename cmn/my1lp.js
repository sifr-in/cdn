...
function setupL3EventListeners() {
    ...
    if (sendOtpBtn) {
        sendOtpBtn.addEventListener('click', debounce(function() {
        if (getOTP()) {
    ...
    if (verifyOtpBtn) {
    verifyOtpBtn.addEventListener('click', debounce(function() {
    ...
    if (completeRegistration) {
        completeRegistration.addEventListener('click', handleCompleteRegistration);
    }
    
    if (loginSubmit) {
        loginSubmit.addEventListener('click', fnloginSubmit);
    }
}
...
if (response.uzr.mk.length > 10) {
    ...
    userData.fnf = response.fnf;
    userData.ffp = response.ffp;
...
/*if (typeof initLoginSystem === 'function') {
  // If the script is loaded after DOM is ready, initialize immediately
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    setTimeout(initLoginSystem, 0);
  } else {
